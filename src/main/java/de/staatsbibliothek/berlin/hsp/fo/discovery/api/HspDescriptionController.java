...
@Validated
public class HspDescriptionController extends BaseEntityController<HspDescription> {

  @Autowired
  public HspDescriptionController(final BaseService<HspDescription> baseService, final HspConfig hspConfig, final HighlightConfig highlightConfig) {
    super(baseService, hspConfig, highlightConfig);
  }

  @ApiResponses(value = {@ApiResponse(content = @Content(schema = @Schema(implementation = HspDescription.class), mediaType = MediaType.APPLICATION_JSON_VALUE), description = "Description for the given id", responseCode = "200"), @ApiResponse(content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE), description = "If no description was found for the given Id", responseCode = "404"), @ApiResponse(content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE), description = "If something went terribly wrong", responseCode = "500")})
  ...
  public ResponseEntity<HspDescription> getById(
      ...
      @RequestParam(defaultValue = "", name = "displayFields", required = false)
      final List<String> displayFields) {
    final Result<List<HspDescription>> result = byId(id, displayFields);
    return new ResponseEntity<>(result.getPayload()
        .get(0), HttpStatus.OK);
  }

  @ApiResponses(value = {@ApiResponse(content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE), description = "All Descriptions within the range of [start, start+rows]", responseCode = "200"), @ApiResponse(content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE), description = "If something went terribly wrong", responseCode = "500")})
  ...
  public ResponseEntity<Result<List<HspDescription>>> getAll(
    ...
    final Result<List<HspDescription>> result =  all(filteredFields, start, rows);
  ...
  public ResponseEntity<Result<List<HspDescription>>> search(
